...
const MatchingRides = () => {
  ...
  useEffect(() => {
    async function fetchRides() {
      try {
        const res = await axios.get(`${apiURL}/rides/matchingrides?destination=${destination}`);
  ...
  return (
    <div>
      ...
      {rides.length === 0 ? (
        ...
        rides.map((ride) => (
          <div key={ride.rideId} style={{ border: "1px solid gray", padding: "10px", margin: "10px" }}>
            <p><b>Driver:</b> {ride.driver.user.name}</p>
            <p><b>Phone:</b> {ride.driver.user.phone}</p>
            <p><b>Seats Available:</b> {ride.seats}</p>
            <p><b>Fare:</b> ₹{ride.fare}</p>
            <button onClick={() => confirmRide(ride.rideId)}>Confirm Ride</button>
